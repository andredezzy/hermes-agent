...
def _effective(raw: Dict[str, Any], config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    # A profile that opted into inheritance carries only its own overrides on disk; the rest
    # lives in the root config. Resolving it here — rather than in any one caller — keeps the
    # gateway, cron and logging agreeing about what the profile's model is. Without it an
    # inheriting profile reads as model-less and each caller falls through to its own default,
    # which is how a room ended up pinned to a model its provider does not serve.
    if merged.get("inherit") is True and config_path is not None:
        inherited = _config._load_inherited_config(config_path)
        inherited.pop("inherit", None)
        if inherited:
            # Inherited values are the BASE; the profile's own file overrides them. The reverse
            # order silently overwrites a profile's deliberate choice with the parent's.
            merged = _config._deep_merge(inherited, merged)
...
def load_user_config_effective(config_path: Optional[Path] = None, *, fail_closed: bool = False) -> Dict[str, Any]:
    ...
    with _config._CONFIG_LOCK:
        ...
        effective = _effective(raw, config_path)
