...
describe('clarify and approvals (#90694)', () => {
  ...
  it.each([
    { kind: 'clarify', payload: CLARIFY, requestId: CLARIFY.id },
    { kind: 'approval', payload: APPROVAL, requestId: APPROVAL.request_id }
  ])('keeps a pending $kind answerable after the room stops waiting', async ({ kind, payload, requestId }) => {
    let clock = Date.now()
    ...
    try {
      ...
      expect(entries[0]).toMatchObject({ kind, requestId })
...
describe('stranded harvest', () => {
  ...
  it.each(['clarify', 'approval'])('recovers a pending %s while the stranded member is still busy', async kind => {
    // Each kind arrives in its own shape: a clarify is an open-request frame
    // keyed on `id`, an approval is a `pending_approval` body keyed on
    // `request_id`. Same question to the user, two wire formats.
    const payload =
      kind === 'clarify'
        ? { id: 'req-pending', method: 'clarify', params: { choices: ['yes', 'no'], question: 'Proceed?' } }
        : { choices: ['yes', 'no'], description: 'Proceed?', question: 'Proceed?', request_id: 'req-pending' }

    const pending = { research: { payload, until: 1 } }
    ...
    expect(room.turns.groupHasPendingClarify(room.chat.$groupClarify.get(), 'Waiting')).toBe(true)
    expect(room.chat.$groupChats.get().Waiting.stranded?.research).toEqual({ before: 0, thread: 'original-thread' })
    expect(room.gateway.rpcFor('prompt.submit')).toHaveLength(0)
