...
from hermes_cli.update_cmd_git import (  # noqa: F401
    ...
    _branch_head_label, _branch_head_suffix, _classify_fetch_failure, _classify_merge_failure,
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if is_shallow:
        ...
        # Differing tips are not proof of being behind: a carried local commit puts HEAD
        # ahead of the fetched tip. Ask git before the compare API, which cannot see a
        # commit that exists only here and would otherwise report a permanent,
        # unclearable "update available" (mirrors the banner's shallow path).
        if head_sha and target_sha and subprocess.run(
...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    if _cur_branch and _cur_branch != branch:
        ...
        if merge_result.returncode != 0:
            ...
            print(_classify_merge_failure(merge_stderr))
            if "unrelated histories" not in merge_stderr:
