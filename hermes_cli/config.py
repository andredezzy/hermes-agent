...
def _load_config_cache_sig(config_path: Path) -> Tuple[Optional[Tuple[int, int, int, int]], Optional[Tuple[int, ...]]]:
    ...
    the merged result, and the inherited root config's the same way — a profile that inherits
    depends on a second file, so a root edit has to invalidate the cache too. The root is stat'd
    unconditionally: reading ``inherit`` would mean parsing the profile config on every load,
    which is the work the cache exists to avoid. ``cache_sig`` is None only when neither file
    exists (nothing to cache on)."""
    try:
        st = config_path.stat()
    ...
    try:
        ...
        root_sig = file_signature(rst) if rst else (0, 0, 0, 0)
    except OSError:
        root_sig = (0, 0, 0, 0)
    if user_sig is None and managed_sig == (0, 0, 0, 0):
        return None, None
    return user_sig, (*(user_sig or (0, 0, 0, 0)), *managed_sig, *root_sig)
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[: len(cache_sig)] == cache_sig:
            ...
            value_at = len(cache_sig)
            env_snapshot = cached[value_at + 1] if len(cached) > value_at + 1 else {}
            if all(_env_ref_lookup(k) == v for k, v in env_snapshot.items()):
                return copy.deepcopy(cached[value_at]) if want_deepcopy else cached[value_at]
