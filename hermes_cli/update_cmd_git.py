...
_FETCH_FAILURE_RULES = (
    # "rate-limited" (hyphenated) is the SSH transport's wording: it carries no HTTP status and
    # closes the ref listing with a prose banner, so matching only "rate limit" missed it and the
    # user got the bare generic message for a condition that clears itself in minutes.
    (lambda s: _has_http_code(s, "429") or "rate limit" in s.lower() or "rate-limited" in s.lower(),
     "✗ GitHub is rate limiting requests or having an outage — try again in 5 minutes."),
...
def _classify_merge_failure(stderr: str) -> str:
    """Say why an in-place merge failed, and what actually fixes it.

    A parked-branch update merges origin/<branch> so local commits survive. Every failure used to
    be reported as a conflict, but on a shallow checkout the far more likely cause is that the
    fetched branch shares no commit with local history — git refuses outright, nothing is
    conflicted, and the advice to "resolve manually" sends the user to the same refusal. The two
    need different remedies: one edits files, the other fetches history.
    """
    if "unrelated histories" in stderr:
        return (
            "✗ The fetched branch shares no history with this checkout — this is a shallow clone"
            " whose history was cut before the common ancestor, not a conflict.\n"
            "  Deepen it, then re-run the update: git fetch --deepen=2000 origin <branch>\n"
            "  (repeat if needed, or 'git fetch --unshallow' to fetch it all)."
        )
    return "✗ Merge conflict between local commits and upstream — update stopped, nothing was changed."
