...
async function pollGroupMemberTurn(context: GroupTurnPollContext): Promise<null | string> {
  ...
  // Timeout — read as a pass, but remember the baseline + thread (runtime-only)
  // so the finished reply can be posted late into the RIGHT thread instead of
  // vanishing. The pending question card STAYS: the backend owns that decision
  // and is still waiting on it, so dismissing it here strands the member on a
  // question the user can no longer answer.
  recordGroupActivity(context.group, {
    kind: 'timed-out',
    member: member.name,
    thread
